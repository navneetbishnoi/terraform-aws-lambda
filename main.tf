...
##-----------------------------------------------------------------------------
# FIXED DEPRECATED WARNING: Region-specific endpoint ki jagah global service name use kiya gaya hai
...
##-----------------------------------------------------------------------------
## OPTION 1 FIX: Naya S3 Access Policy Document aur Resources (Yeh sabse important fix hai!)
##-----------------------------------------------------------------------------
data "aws_iam_policy_document" "s3_code_access" {
  count = var.enable && var.create_iam_role ? 1 : 0

  statement {
    effect  = "Allow"
    actions = ["s3:GetObject"]
    # Apna exact bucket aur file ka ARN daalein
    resources = ["arn:aws:s3:::bucket-test1/index.zip"]
  }
}

# Naya S3 Policy Resource
resource "aws_iam_policy" "s3_code_access" {
  count       = var.enable && var.create_iam_role ? 1 : 0
  name        = format("%s-s3-code-access", module.labels.id)
  path        = var.policy_path
  description = "Allows Lambda to download code from S3"
  policy      = data.aws_iam_policy_document.s3_code_access[0].json
  tags        = module.labels.tags
}

# Nayi S3 Policy ko Role se Attach Karen
resource "aws_iam_role_policy_attachment" "s3_code_access" {
  count      = var.enable && var.create_iam_role ? 1 : 0
  role       = aws_iam_role.default[0].name
  policy_arn = aws_iam_policy.s3_code_access[0].arn
}